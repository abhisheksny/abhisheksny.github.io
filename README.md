# Abhishek Kumar

**Biotechnology · Immunoinformatics · Computational Biology**

Personal research website and portfolio of **Abhishek Kumar**, focused on computational biology, structural bioinformatics, and scientific programming.

🌐 **Website:** [abhisheksny.github.io](https://abhisheksny.github.io)

## Research Interests

* Computational Biology
* Structural Bioinformatics
* Host–Pathogen Biology
* Immunoinformatics
* Omics & NGS Analysis
* Molecular Dynamics
* Scientific Programming

## Selected Projects

### FASTA Toolkit

Python/Biopython-based toolkit for sequence analysis, including FASTA statistics, ORF identification and protein translation.

[View repository](https://github.com/abhisheksny/bio_project)

### FACS Data Analysis

Python-based workflow for processing FCS files, generating visualizations and extracting analytical metrics from flow-cytometry data.

### qPCR Data Analysis

Analysis workflow for HPV qPCR data, including quality control, Ct-based interpretation and summary statistics.

### Protein–Ligand Simulation

Computational structural-biology workflows involving molecular docking, GROMACS molecular dynamics simulations and trajectory analysis.

## Publication

**Singh PN, Kumar A*, Kumari S, Bhat AH, Alti D.**

*Neem derivatives in integrated control of vectors and pathogens of leishmaniasis: Current knowledge and computational insights for future perspectives.*

**Asian Pacific Journal of Tropical Biomedicine, 2026.**

* Joint first author

## Academic Profiles

* **ORCID:** [0009-0000-2206-0993](https://orcid.org/0009-0000-2206-0993)


---

This repository contains the source files for my personal website, hosted using GitHub Pages.

